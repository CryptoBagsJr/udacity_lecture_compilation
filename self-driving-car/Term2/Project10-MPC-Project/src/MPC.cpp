...
const double ref_v = 90;  // Highest speed

// Index
const double x_start = 0;
const double y_start = x_start + N;
const double psi_start = y_start + N;
...
const double BOUNDARY = 1.0e19;
const double DEGREE = 0.436332;
...
class FG_eval {
 public:
  ...
  void operator()(ADvector& fg, const ADvector& vars) {
    ...
    for (int i = 0; i < N - 2; i++) {
      fg[0] += WEIGHT_DIFF_DELTA * CppAD::pow(vars[delta_start + i + 1] - vars[delta_start + i], 2);
      fg[0] += WEIGHT_DIFF_A * CppAD::pow(vars[a_start + i + 1] - vars[a_start + i], 2);
    }
...
vector<double> MPC::Solve(Eigen::VectorXd state, Eigen::VectorXd coeffs) {
  ...
  for ( int i = delta_start; i < a_start; i++ ) {
    vars_lowerbound[i] = -DEGREE * Lf;
    vars_upperbound[i] = DEGREE * Lf;
  }
